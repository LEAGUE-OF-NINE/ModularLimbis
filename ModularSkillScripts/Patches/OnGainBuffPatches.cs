...
internal class OnGainBuffPatches
{
	...
	[HarmonyPostfix]
	private static bool Postfix_BattleUnitModel_RightAfterGetAnyBuff(BUFF_UNIQUE_KEYWORD keyword, int stack, int turn, int activeRound, ABILITY_SOURCE_TYPE srcType, BATTLE_EVENT_TIMING timing, BattleUnitModel giverOrNull, BattleActionModel actionOrNull, int overStack, int overTurn, BattleUnitModel __instance)
    {        
        int actevent = MainClass.timingDict["OnGainBuff"];
        if (!__instance.IsActionable()) return true;
        bool noSkip = true;
